    ...
    if file then
        for line in file:lines() do
            ...
            if key and value then
                ...
                elseif key == "hide_at_startup" then config.hide_at_startup = (value == "true")
                end
            end
        end
        file:close()
...
local function save_config()
    ...
    if file then
        ...
        file:write("hide_at_startup=" .. tostring(config.hide_at_startup) .. "\n")
        file:close()
    end
end
...
local function connect()
    ...
    if success then
        ...
        log_message("Connected to Transmitter server: " .. connection_host .. ":" .. connection_port)
        XPLMSpeakString("Connected to Transmitter server")
    else
...
function draw_window()
    ...
    -- Hide at startup checkbox
    local changed
    changed, config.hide_at_startup = imgui.Checkbox("Connect and hide at startup", config.hide_at_startup)
    if changed then
        save_config()
    end
end
...
-- Check if we have valid config for auto-connect
local function has_valid_config()
    if config.server_url == "" then
        return false -- Server URL is empty
    end
    if config.callsign == "" or config.callsign == "CALLSIGN" then
        return false -- Callsign is default or empty
    end
    return true
end

-- Handle startup behavior
if config.hide_at_startup and has_valid_config() then
    -- Auto-connect and stay hidden
    connect()
else
    -- Show the window
